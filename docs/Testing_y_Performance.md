...
|------|-------------|-----------|
| Tests unitarios e integración | Vitest + Testing Library | **16 tests** en 5 archivos — todos pasan |
| Performance (Lighthouse) | Chrome / Edge, Mobile y Desktop | Todos **> 80** (ver §3) |
...
|---------|------|------------|
| `tests/dogApi.test.js` | Unitario | Fetch a Dog CEO API, catálogo por raza, pool de 12 imágenes, mapeo de URLs a fotos |
...
- Header muestra el título "Patitas"

**`App` — integración** (6 tests)
...
- Agrega favorito desde home y lo muestra en galería
- Muestra estado vacío en galería sin favoritos
- Cambia de raza y vuelve a llamar a la API
- No vuelve a pedir imágenes al regresar del home desde galería (caché de sesión)
...
3. **Abrir Chrome o Edge** en esa URL (idealmente ventana de incógnito, sin extensiones que alteren la medición).

4. **DevTools → Lighthouse** (F12 → pestaña *Lighthouse*):

   - Modo: **Navigation**
   - Categorías: al menos **Performance** (recomendado marcar todas)
   - Dispositivo: **Mobile** o **Desktop** (repetir en cada navegador que quieras documentar)

5. **Analyze page load** y exportar el reporte (HTML o JSON) a `docs/` si se repite la medición.

> **Nota:** Los puntajes varían según navegador y perfil de dispositivo. En pantallas grandes suelen entrar más imágenes visibles a la vez (12 en grilla 4×3), lo que puede bajar Performance en Desktop respecto a Mobile.

### Resultados registrados (mayo 2026)

Medición sobre `http://localhost:4173/` tras `npm run build` + `npm run preview`. Criterio del curso: **Performance > 80** en todas las corridas.

| Navegador | Mobile | Desktop |
|-----------|--------|---------|
| **Microsoft Edge** | **94** | **95** |
| **Google Chrome** | **90** | **86** |

Reporte Lighthouse de referencia (Chrome, Mobile, medición anterior): [`lighthouse-report.report.html`](lighthouse-report.report.html) / [`.json`](lighthouse-report.report.json). Las capturas de Edge y Chrome (Mobile/Desktop) van en el informe de entrega.
...
- [ ] Captura de terminal con `npm run test:run` — 16/16 tests OK
- [ ] Capturas de Lighthouse Performance **> 80** (Chrome y Edge; Mobile y Desktop; build + preview)
